...
async function consultarTodasLasAPIs() {
  ...
  for (const pais of PAISES) {
    try {
      ...
      const resumen = canalesConFallas.length === 0 
        ? "Todos estables ✅" 
        : `${canalesConFallas.length} canales con fallas`;

      mensajes.push(`*${escapeMarkdownV2(pais.nombre)}*: ${escapeMarkdownV2(resumen)}`);
    } catch (error) {
      mensajes.push(`*${escapeMarkdownV2(pais.nombre)}*: Error al consultar API (${escapeMarkdownV2(error.message)})`);
    }
  }
...
async function enviarTelegram(texto) {
  try {
    ...
    await axios.post(url, { chat_id: CHAT_ID, text: texto, parse_mode: "MarkdownV2" });
  } catch (error) {
