...
async function consultarTodasLasAPIs() {
  ...
  for (const pais of PAISES) {
    try {
      ...
      // Solo canales que NO tengan "(1)"
      const canalesConFallas = todosCanales.filter(canal => /\(\d+\)/.test(canal) && !canal.includes("(1)"));

      if (canalesConFallas.length === 0) {
        mensajes.push(`*${escapeMarkdownV2(pais.nombre)}*: ESTABLE`);
      } else {
        mensajes.push(`*${escapeMarkdownV2(pais.nombre)}*:\n${canalesConFallas.map(escapeMarkdownV2).join("\n")}`);
      }

    } catch (error) {
      mensajes.push(`*${escapeMarkdownV2(pais.nombre)}*:\nError al consultar API (${escapeMarkdownV2(error.message)})`);
    }
  }
