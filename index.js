...
async function consultarTodasLasAPIs() {
    ...
    for (const pais of PAISES) {
        try {
            ...
            // Tomar solo arrays no vacíos y aplanarlos
            const canales = data.filter(Array.isArray).flat();

            // Filtrar solo los que tengan falla: suponiendo que "(0)" indica error
            const canalesConFallas = canales.filter(canal => canal.includes("(0)"));
